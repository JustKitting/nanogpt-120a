...
pub fn score_candidate(
    ...
    let expected_speed = survival_prior * speed;
    let mut uncertainty = [predicted_quality, predicted_stability]
        .into_iter()
        .flatten()
        .map(|p| p.uncertainty)
        .fold(0.0, f64::max);
    if config.sweep_speed_weight != 0.0 {
        if let Some(speed) = predicted_speed {
            uncertainty = uncertainty.max(speed.uncertainty);
        }
    }
    let exploration = uncertainty.ln_1p();
    let score = config.sweep_quality_weight * expected_quality
        + config.sweep_speed_weight * expected_speed
