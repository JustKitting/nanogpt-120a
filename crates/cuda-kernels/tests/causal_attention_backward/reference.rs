...
pub fn case() -> Case {
    ...
    let (out, log_sum_exp) = forward(&qkv);
    let expected = backward(&qkv, &out, &d_out, &log_sum_exp);
    Case {
        qkv,
        d_out,
        out,
...
fn forward(qkv: &[f32]) -> (Vec<f32>, Vec<f32>) {
    ...
    let mut log_sum_exp = vec![0.0_f32; TOKEN_COUNT * HEADS];
    for query in 0..TOKEN_COUNT {
        for head in 0..HEADS {
            let scores = scores(qkv, query, head);
            let row_log_sum_exp = logsumexp(&scores);
            log_sum_exp[log_sum_exp_index(query, head)] = row_log_sum_exp;
            for dim in 0..HEAD_DIM {
                let mut value = 0.0;
                for key in 0..=query {
                    let p = (scores[key] - row_log_sum_exp).exp();
