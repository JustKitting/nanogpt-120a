...
#[inline(always)]
pub(super) fn store_add_in_place(
    acc: [f32; 4],
    tile: CtaTile,
    warp_n: u32,
    ...
    store_four!(store_one, acc, tile, warp_n, out, rows, cols, scales);
}
...
fn store_one(
    ...
    if row < rows && col < cols {
        ...
        unsafe {
            let value = out.get_unchecked_mut(offset);
            *value = scales.base * *value + scales.matmul * acc;
        }
    }
}
