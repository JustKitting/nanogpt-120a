...
#[inline(always)]
pub fn affine_value(
    acc: f32, row: u32, col: u32, input_global_scales: &[f32], bias_bytes: &[u8], bias_scales: &[u8],
...
pub fn affine_value_scaled(
    acc: f32, global_scale: f32, col: u32, bias_bytes: &[u8], bias_scales: &[u8],
    params: &Nvfp4ProjectionParams,
) -> f32 {
    let bias = nvfp4_value(bias_bytes, bias_scales, params.bias_global_scale, col as usize);
    fma_f32(acc, global_scale, bias)
}

#[inline(always)]
pub fn affine_pair_scaled(
    acc: (f32, f32), scale: f32, col0: u32, bias: (&[u8], &[u8]),
    params: &Nvfp4ProjectionParams,
) -> (f32, f32) {
    (
