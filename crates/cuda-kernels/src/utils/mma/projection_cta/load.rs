...
use super::tile::{
    ...
    NVFP4_PROJECTION_CTA_B_SCALES, NVFP4_PROJECTION_CTA_PACKS_PER_ROW, Nvfp4ProjectionCtaTile,
};
...
pub fn load_a_fragments(
    ...
    [
        load_a_fragment(a_packs, tile, 0),
        load_a_fragment(a_packs, tile, 1),
        load_a_fragment(a_packs, tile, 2),
        load_a_fragment(a_packs, tile, 3),
    ]
}
...
pub fn load_b_fragments(
    ...
    [
        load_b_fragment(b_packs, tile, 0),
        load_b_fragment(b_packs, tile, 1),
    ]
}
...
pub fn load_b_scale4(
    ...
    b_scales[(tile.warp_n * 8 + tile.group) as usize]
}
...
fn load_a_fragment(
    ...
    let pack = tile.thread_in_group + if register < 2 { 0 } else { 4 };
...
fn load_b_fragment(
    ...
    let pack = tile.thread_in_group + if register == 0 { 0 } else { 4 };
