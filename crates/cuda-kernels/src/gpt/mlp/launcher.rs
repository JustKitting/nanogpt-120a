...
use super::args::{MlpDownResidualArgs, MlpUpRelu2Args, MlpUpRelu2TapeArgs};
use super::kernels;
use crate::mma::{
    NVFP4_PROJECTION_ACTIVATION_NONE, NVFP4_PROJECTION_ACTIVATION_RELU2,
...
impl MlpModule {
    ...
    pub fn up_relu2(&self, args: MlpUpRelu2Args<'_, '_>) -> Result<(), DriverError> {
        self.module.mlp_projection_kernel(
            ...
            args.bias.scales,
            args.out,
            params(&args, 0, NVFP4_PROJECTION_ACTIVATION_RELU2),
        )
    }

    pub fn up_relu2_tape(&self, args: MlpUpRelu2TapeArgs<'_, '_>) -> Result<(), DriverError> {
        self.module.mlp_projection_relu2_tape_kernel(
            args.stream,
            config(args.token_count, args.output_dim),
            args.input.bytes,
            args.input.scales,
...
fn tape_params(args: &MlpUpRelu2TapeArgs<'_, '_>) -> Nvfp4ProjectionParams {
    Nvfp4ProjectionParams {
        token_count: args.token_count,
        input_dim: args.input_dim,
        output_dim: args.output_dim,
