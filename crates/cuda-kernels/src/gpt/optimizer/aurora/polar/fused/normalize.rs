...
pub(super) fn normalize_source_to_x(
    ...
    let len = source_rows * source_cols;
    ...
    normalize_source_to_x_from_chunks(
        source,
        x,
        chunks,
        warp_sums,
...
fn normalize_source_to_x_from_chunks(
    ...
    let polar_len = polar_rows * polar_cols;
    let mut polar_index = work.thread();
    while polar_index < polar_len {
        let src = source_index(polar_index, source_rows, source_cols, transpose_source);
        write_f32(x, polar_index, read_f32(source, src) * inv_norm);
        polar_index += stride;
    }
}
...
fn source_index(
    ...
    if transpose_source {
        let row = polar_index / source_rows;
        let col = polar_index - row * source_rows;
        col * source_cols + row
    } else {
        polar_index
    }
